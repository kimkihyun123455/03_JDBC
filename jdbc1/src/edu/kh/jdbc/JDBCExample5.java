...
public class JDBCExample5 {
	...
	public static void main(String[] args) {
		...
		// SELECT가 아니기 때문에 ResultSet은 필요 없다
		
		Scanner sc = null;
		
		try {
			
			// 2. DriveManager 를 이용해 Connection 객체 생성
			...
			// + DML 수행전에 해줘야 할 것
			// AutoCommit 끄기
			// -> 개발자가 트랜잭션을 마음대로 제어하기 위해 자동커밋 종료
			conn.setAutoCommit(false);
			
			// 6. SQL (INSERT) 수행 후 결과(int) 반환 받기
			// executeUpdate() : DML 수행, 결과 행 갯수(int) 반환
			// exectueQuery() : SELECT 수행, ResultSet 반환
			
			// pstmt에서 executeQuery(), executeUpdate() 매개변수 자는 공석
